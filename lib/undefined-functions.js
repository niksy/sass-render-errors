 ...
 * @param {string} output
 */
function isUndefinedFunction(input, output) {
	const parsedInput = parse(input || String(null));
	const parsedOutput = parse(output || String(null));

	/** @type {string[]} */
	const matchingPairs = [];

	[parsedInput, parsedOutput].forEach((parsed) => {
	...
	try {
		...
		parseRoot.walkRules(nodeSelectorRegex, (rule) => {
			...
			rule.walkDecls((decl) => {
				...
				} else if (decl.prop === '-lint-output') {
					node[decl.prop] = String(
						JSON.parse(decl.value).replace(id, '')
					);
				} else if (decl.prop === '-lint-input') {
					node[decl.prop] = String(JSON.parse(decl.value));
				} else if (decl.prop === '-lint-name') {
					node[decl.prop] = String(JSON.parse(decl.value));
				}
			});

			if (
				isUndefinedFunction(node['-lint-input'], node['-lint-output'])
			) {
