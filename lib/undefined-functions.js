...
/**
 * @import {SassModule, Sass, SassRenderError, SassRenderer} from './types.ts'
 ...
 * @param {SassRenderer<T>}                        renderer
 * @param {{type: "file"|"code", value: string}}   input
 * @param {string[]}                               knownCssFunctions
 * @param {ReturnType<typeof createJsonFunctions>} jsonFunctions
 */
async function getRenderResults(renderer, input, knownCssFunctions, jsonFunctions) {
	...
	try {
		...
		const defaultOptions = {
			functions: {
				[`${id}${jsonEncodeFunctionKey}`]: jsonFunctions[jsonEncodeFunctionKey]
			},
			verbose: true,
			...(entryUrl && {
				url: entryUrl
...
export default function (sass, options) {
	...
	const jsonFunctions = createJsonFunctions(sass);
	...
	return {
		 ...
		 * @param {Sass.Options<"sync">=} options Sass options.
		 */
		 ...
		 *
		 * @param {string}                 input   File to process.
		 * @param {Sass.Options<"async">=} options Sass options.
		 */
		 ...
		 * @param {Sass.StringOptions<"sync">=} options Sass options.
		 */
		 ...
		 *
		 * @param {string}                       input   String to process.
		 * @param {Sass.StringOptions<"async">=} options Sass options.
		 */
