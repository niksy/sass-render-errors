...
const outputRegexes = [
	/(?<type>Error|DEPRECATION WARNING):(?<message>.+?)╷\s*?\d+\s*│(?<codeLine>\s*.+?)\n\s*│(?<codePositionMarker>\s*\^+?)\n\s*╵\s*(?<file>.+?)\s(?<line>\d+):(?<column>\d+)/gs,
	/(?<type>DEPRECATION WARNING) on line (?<line>\d+), column (?<column>\d+) of (?<file>.+?):(?<message>.+?)╷\s*?\d+\s*│(?<codeLine>\s*.+?)\n\s*│(?<codePositionMarker>\s*\^+?)\n/gs,
...
function parseErrorOutput(input, file, cwd) {
	...
	results.forEach((result) => {
		let composedFile = file ?? result.file;
		if (composedFile !== 'stdin') {
			composedFile = path.resolve(cwd, result.file);
		}
		errors.push({
			...result,
			type: 'error',
